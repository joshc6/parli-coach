...
const s = {
  ...
  emptyState: { display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:"100%", textAlign:"center", gap:8 },
  emptyIcon: { fontSize:48 },
  emptyTitle: { fontSize:22, fontWeight:700, margin:0, letterSpacing:"-0.02em" },
  emptyDesc: { fontSize:13, color:"#94a3b8", maxWidth:280, lineHeight:1.6, margin:0 },
  emptyHint: { fontSize:13, color:"#4f46e5", fontWeight:600, margin:0 },
...
export default function App() {
  ...
  // ── KEY CHANGE: hit /api/claude (our Vercel proxy) instead of Anthropic directly ──
  const sendMessage = async (messageText, isFeedback = false, isPracticeRefutation = false) => {
    ...
    try {
      ...
      // Calls our serverless proxy — API key never touches the browser
  ...
  const handleQuickAction = (action) => {
    ...
    if (action === "practice") {
      ...
      sendMessage(`You are now in practice debate mode. Present ONE strong argument that the ${side === "gov" ? "Opposition" : "Government"} side would make against this resolution. Argue it persuasively and make it genuinely challenging — include their Claim, Warrant, and Impact clearly. Do NOT provide any refutation or hint at how to beat it. Stop after presenting the argument and wait for the user's response.`);
      return;
    }
      ...
      <div style={s.body}>
        ...
        <aside style={s.sidebar}>
          ...
          <div>
            ...
            />
            {!resolutionSet
              ? <button style={s.btnPrimary} onClick={handleSetResolution} disabled={!resolution.trim()}>Set Resolution</button>
              : <button style={s.btnSecondary} onClick={() => { setResolutionSet(false); setSide(null); setMessages([]); setPracticeMode(false); setPracticeStage("idle"); }}>Change</button>
            }
          </div>
          ...
          {resolutionSet && side && (
            <div>
              ...
              {[
                ...
                { key:"practice", icon:"⚔️", label:"Practice Mode" },
                { key:"plan", icon:"📄", label:"Write a Plan" },
                { key:"counterplan", icon:"↩️", label:"Counterplan" },
              ].map(({ key, icon, label }) => (
                <button
                  key={key}
                  style={key === "practice" && practiceMode ? s.actionBtnPracticeActive : s.actionBtn}
        ...
        <main style={s.chat}>
          <div style={s.messages}>
            {messages.length === 0 && (
              <div style={s.emptyState}>
                ...
                <p style={s.emptyTitle}>Ready to debate.</p>
                <p style={s.emptyDesc}>Set a resolution on the left, pick your side, then generate a case or ask anything.</p>
                {!resolutionSet && <p style={s.emptyHint}>← Start with a resolution</p>}
                {resolutionSet && !side && <p style={s.emptyHint}>← Pick Government or Opposition</p>}
              </div>
            )}
                    ...
                    <div style={msg.isOpponentArg ? s.msgActionsRose : msg.isFeedbackResponse ? s.msgActionsGreen : s.msgActions}>
                      ...
                      {msg.isFeedbackResponse && (
                        <>
                          ...
                          <button style={s.smallBtnGray} onClick={() => { setPracticeMode(false); setPracticeStage("idle"); }}>Exit Practice</button>
                        </>
                      )}
                      {!msg.isOpponentArg && (
                        <>
            ...
            <div style={s.revBar}>
              ...
              <div style={s.revRow}>
                <textarea
                  ...
                  rows={2}
                  placeholder="What should change? e.g. 'Make contention 2 stronger', 'Rewrite the intro'..."
                  value={feedbackText}
          ...
          <div style={s.inputBar}>
            ...
            {!resolutionSet && <div style={{ fontSize:12, color:"#94a3b8", marginBottom:6 }}>Set a resolution to begin.</div>}
            <div style={s.inputRow}>
              <textarea
                style={s.inputTextarea}
                rows={2}
                placeholder={
                  !resolutionSet ? "Set a resolution to begin..."
                  : !side ? "Pick a side first..."
                  : isAwaitingRefutation ? "Type your refutation here..."
                  : "Ask anything — refutations, arguments, plans, counterplans..."
                }
                value={input}
